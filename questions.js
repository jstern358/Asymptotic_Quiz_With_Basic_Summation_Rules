window.TrainerData = (() => {
  ...
  function fmtTheta(s) { return `\\(\\Theta(${s})\\)`; }
  ...
  function makeExplanation(blockSummaries, steps, finalRuntime, detailedTypeSteps = []) {
    return { blockSummaries, steps, finalRuntime, detailedTypeSteps };
  }
  ...
  function simplifyExpDecSumStep(indexVar) {
    return {
      title: 'Simplify the sum',
      content:
        `Factor out the n from the numerator first: ` +
        `\\[\\sum_{${indexVar}=0}^{\\lfloor \\log_2 n \\rfloor} \\frac{n}{2^${indexVar}} = n\\sum_{${indexVar}=0}^{\\lfloor \\log_2 n \\rfloor} 2^{-${indexVar}}.\\] ` +
        `Now the simplified summation to classify is \\(\\sum 2^{-${indexVar}}\\), not the original sum with n still inside.`
    };
  }

  function simplifyExpIncSumStep(indexVar, base) {
    return {
      title: 'Simplify the sum',
      content:
        `The summation is already in canonical exponentially-increasing form: ` +
        `\\[\\sum_{${indexVar}=0}^{\\lfloor \\log_2 n \\rfloor} ${base}^{${indexVar}}.\\] ` +
        `The key term for evaluation is the last term \\(${base}^{\\lfloor \\log_2 n \\rfloor}\\).`
    };
  }

  function polynomialTypeSummary(indexVar, power) {
    const term = power === 1 ? `${indexVar}` : `${indexVar}^{${power}}`;
    return {
      title: 'Determine the summation type',
      content:
        `After simplification, the canonical summand is \\(f(${indexVar})=${term}\\). ` +
        `This is a polynomial-type summand, so ` +
        `\\[\\sum_{${indexVar}=1}^{n} f(${indexVar}) = \\Theta\\!\\bigl(n f(n)\\bigr).\\]`
    };
  }

  function expIncreasingTypeSummary(indexVar, base) {
    return {
      title: 'Determine the summation type',
      content:
        `After simplification, the canonical summand is \\(f(${indexVar})=${base}^{${indexVar}}\\). ` +
        `This is of exponentially-increasing type, so the sum is asymptotically the same order as its last term.`
    };
  }

  function expDecreasingTypeSummary(indexVar) {
    return {
      title: 'Determine the summation type',
      content:
        `After simplification, the canonical summand is \\(f(${indexVar})=2^{-${indexVar}}\\). ` +
        `This is of exponentially-decreasing type, so ` +
        `\\[\\sum f(${indexVar}) = \\Theta(1).\\]`
    };
  }

  function provePolynomialType(indexVar, power) {
    const term = power === 1 ? `${indexVar}` : `${indexVar}^{${power}}`;
    const halfTerm = power === 1 ? `${indexVar}/2` : `(${indexVar}/2)^{${power}}`;
    const factor = 2 ** power;
    return {
      title: 'Full summation type analysis: polynomial type',
      content:
        `Let \\(f(${indexVar})=${term}\\). This function is nonnegative and nondecreasing for sufficiently large ${indexVar}. ` +
        `Also, ` +
        `\\[f(${indexVar})=${term}=${factor}\\cdot f(${indexVar}/2)\\] ` +
        `because \\(f(${indexVar}/2)=${halfTerm}\\). ` +
        `Hence there exists a constant \\(c>1\\) (for example, \\(c=${factor}\\)) such that ` +
        `\\[f(${indexVar}) \\le c\\,f(${indexVar}/2)\\] ` +
        `for all sufficiently large ${indexVar}. Therefore the simplified summand is of polynomial type.`
    };
  }

  function proveExpIncreasingType(indexVar, base) {
    return {
      title: 'Full summation type analysis: exponentially-increasing type',
      content:
        `Let \\(f(${indexVar})=${base}^{${indexVar}}\\). Choose \\(k=1\\) and \\(c=${base}>1\\). Then ` +
        `\\[f(${indexVar})=${base}^{${indexVar}}=${base}\\cdot ${base}^{${indexVar}-1}=c\\,f(${indexVar}-1).\\] ` +
        `So for all sufficiently large ${indexVar}, ` +
        `\\[f(${indexVar}) \\ge c\\,f(${indexVar}-k).\\] ` +
        `Therefore the simplified summand is of exponentially-increasing type.`
    };
  }

  function proveExpDecreasingType(indexVar) {
    return {
      title: 'Full summation type analysis: exponentially-decreasing type',
      content:
        `After factoring out the external multiplicative constant, let \\(f(${indexVar})=2^{-${indexVar}}=1/2^{${indexVar}}\\). ` +
        `Choose \\(k=1\\) and \\(c=1/2\\), where \\(0<c<1\\). Then ` +
        `\\[f(${indexVar})=2^{-${indexVar}}=\\frac{1}{2}\\cdot 2^{-(${indexVar}-1)}=c\\,f(${indexVar}-1).\\] ` +
        `So for all sufficiently large ${indexVar}, ` +
        `\\[f(${indexVar}) \\le c\\,f(${indexVar}-k).\\] ` +
        `Therefore the simplified summand is of exponentially-decreasing type.`
    };
  }
  ...
  function triangularExplanation(i) {
    return makeExplanation(
      ...
      [
        ...
        polynomialTypeSummary(i, 1),
        { title: 'Evaluate the canonical sum', content: `${i} is a degree-1 polynomial summand, so \\(\\sum_{${i}=1}^{n} ${i}=\\Theta(n^2)\\).` },
        { title: 'Final runtime', content: '\\[T(n)=\\Theta(n^2)\\]' }
      ],
      fmtTheta('n^2'),
      [provePolynomialType(i, 1)]
    );
  }
  ...
  function polyInnerExplanation(i, power) {
    return makeExplanation(
      ...
      [
        ...
        polynomialTypeSummary(i, power),
        { title: 'Evaluate the canonical sum', content: `This is a polynomial-type sum of degree ${power}, so \\(\\sum_{${i}=1}^{n} ${i}^{${power}}=\\Theta(n^{${power + 1}})\\).` },
        { title: 'Final runtime', content: `\\[T(n)=\\Theta(n^{${power + 1}})\\]` }
      ],
      fmtTheta(`n^${power + 1}`),
      [provePolynomialType(i, power)]
    );
  }
  ...
  function shiftedTriangularExplanation(i, j) {
    return makeExplanation(
      ...
      [
        ...
        polynomialTypeSummary(i, 1),
        { title: 'Evaluate the canonical sum', content: `\\[\\sum_{${i}=3}^{n-2} (${i}-2)=\\Theta\\!\\left(\\sum_{${i}=1}^{n} ${i}\\right)=\\Theta(n^2)\\]` },
        { title: 'Final runtime', content: '\\[T(n)=\\Theta(n^2)\\]' }
      ],
      fmtTheta('n^2'),
      [provePolynomialType(i, 1)]
  ...
  function scaledPolynomialExplanation(i) {
    return makeExplanation(
      ...
      [
        ...
        polynomialTypeSummary(i, 2),
        { title: 'Evaluate the canonical sum', content: `\\[\\sum_{${i}=1}^{\\lfloor n/2 \\rfloor-3} ${i}^2=\\Theta\\!\\left(\\sum_{${i}=1}^{n} ${i}^2\\right)=\\Theta(n^3)\\]` },
        { title: 'Final runtime', content: '\\[T(n)=\\Theta(n^3)\\]' }
      ],
      fmtTheta('n^3'),
      [provePolynomialType(i, 2)]
    );
  }
  ...
  function expIncreasingSumExplanation(i) {
    return makeExplanation(
      ...
      [
        ...
        simplifyExpIncSumStep(i, 2),
        expIncreasingTypeSummary(i, 2),
        { title: 'Evaluate the canonical sum', content: 'For an exponentially-increasing sum, the last term dominates, so the sum is \\(\\Theta(2^{\\lfloor \\log_2 n \\rfloor})=\\Theta(n)\\).' },
        { title: 'Final runtime', content: '\\[T(n)=\\Theta(n)\\]' }
      ],
      fmtTheta('n'),
      [proveExpIncreasingType(i, 2)]
    );
  }
  ...
  function expDecreasingSumExplanation(t) {
    return makeExplanation(
      ...
      [
        ...
        { title: 'Exact runtime', content: `\\[T(n)=\\Theta(1)+\\Theta(\\log n)+\\Theta\\!\\left(\\sum_{${t}=0}^{\\lfloor \\log_2 n \\rfloor} \\frac{n}{2^${t}}\\right)\\]` },
        simplifyExpDecSumStep(t),
        expDecreasingTypeSummary(t),
        { title: 'Evaluate the canonical sum', content: `Because \\(f(${t})=2^{-${t}}\\) is of exponentially-decreasing type, ` +
          `\\[\\sum_{${t}=0}^{\\lfloor \\log_2 n \\rfloor} 2^{-${t}}=\\Theta(1).\\] ` +
          `Therefore ` +
          `\\[n\\sum_{${t}=0}^{\\lfloor \\log_2 n \\rfloor} 2^{-${t}} = n\\cdot\\Theta(1)=\\Theta(n).\\]` },
        { title: 'Final runtime', content: '\\[T(n)=\\Theta(1)+\\Theta(\\log n)+\\Theta(n)=\\Theta(n)\\]' }
      ],
      fmtTheta('n'),
      [proveExpDecreasingType(t)]
    );
  }
  ...
  function mixedDominanceExplanation(i) {
    return makeExplanation(
      ...
      [
        ...
        { title: 'Determine the summation type', content: `The dominant canonical summand is \\(f(${i})=${i}^2\\), which is of polynomial type.` },
  ...
  function qLogGrowth() {
    const v = varSet().a; const mult = sample([2, 3]);
    return makeQuestion({
  ...
  function qSequentialLinearTriangular() {
    ...
    return makeQuestion({
      ...
      blocks: [{ label: 'Block 1', start: 1, end: 2, color: '#10b981' }, { label: 'Block 2', start: 4, end: 4, color: '#60a5fa' }, { label: 'Block 3', start: 5, end: 6, color: '#f59e0b' }],
      answer: fmtTheta('n^2'),
      choices: shuffledChoices(fmtTheta('n^2'), [fmtTheta('n'), fmtTheta('n\\log n'), fmtTheta('n^3')]),
  ...
  function qOuterNInnerLogN() {
    ...
    return makeQuestion({
      ...
      blocks: [{ label: 'Block 1', start: 1, end: 1, color: '#60a5fa' }, { label: 'Block 2', start: 2, end: 5, color: '#8b5cf6' }],
  ...
  function qLogOuterLinearInner() {
    ...
    return makeQuestion({
      ...
      blocks: [{ label: 'Block 1', start: 1, end: 1, color: '#10b981' }, { label: 'Block 2', start: 2, end: 5, color: '#8b5cf6' }, { label: 'Block 3', start: 3, end: 4, color: '#f59e0b' }],
      answer: fmtTheta('n\\log n'),
      choices: shuffledChoices(fmtTheta('n\\log n'), [fmtTheta('\\log n'), fmtTheta('n'), fmtTheta('n^2')]),
  ...
  function qSequentialQuadraticPlusLinear() {
    ...
    return makeQuestion({
      ...
      blocks: [{ label: 'Block 1', start: 1, end: 3, color: '#f59e0b' }, { label: 'Block 2', start: 5, end: 6, color: '#10b981' }],
      answer: fmtTheta('n^3'),
      choices: shuffledChoices(fmtTheta('n^3'), [fmtTheta('n^2'), fmtTheta('n\\log n'), fmtTheta('n^4')]),
  ...
  function qTripleDependent() {
    ...
    return makeQuestion({
      ...
      blocks: [{ label: 'Block 1', start: 1, end: 1, color: '#60a5fa' }, { label: 'Block 2', start: 2, end: 2, color: '#0ea5e9' }, { label: 'Block 3', start: 3, end: 4, color: '#f59e0b' }],
      answer: fmtTheta('n^3'),
